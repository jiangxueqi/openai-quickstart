...
class Writer:
    ...
    def save_translated_book(self, book, ouput_file_format):
        logger.debug(ouput_file_format)
        ...
        if ouput_file_format.lower() == "pdf":
            ...
            logger.error(f"不支持文件类型: {ouput_file_format}")
            return ""

        logger.info(f"翻译完成，文件保存至: {output_file_path}")

        return output_file_path


    def _save_translated_book_pdf(self, book):
